 ...
 * @param {boolean} [options.vaultManager.preventDeletingPaymentMethodWithSubscription=false] This param is in beta and the name of the param or the UI of the feature may change in the future. Whether or not to prevent a payment method that is associted with a subscription from being deleted in edit mode. Note: if a payment method without a subscription is updated to be associated with a subscription on the merchant server while the customer is in edit mode, and the payment method is deleted on the client, it will _still_ be deleted.
 *
